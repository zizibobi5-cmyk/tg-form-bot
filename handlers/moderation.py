...
async def cb_accept(
    ...
    # Прим.: Telegram не разрешает обычным (не-Premium) ботам публиковать
    # custom_emoji entities в каналах ни через send_message, ни через
    # copy_message. Единственный способ — Telegram Premium на сам бот.
    # Пока что просто публикуем композированную карточку.
