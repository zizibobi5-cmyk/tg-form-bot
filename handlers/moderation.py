...
async def cb_accept(
    ...
    if settings.userbot_enabled:
        ...
        # Legacy-фоллбэк: копируем поля с премиум-эмодзи через copy_message.
        # На каналах это не работает (Telegram режет custom_emoji при публикации
        # от бота), но в группах помогает — оставляем как есть.
